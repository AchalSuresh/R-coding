...
# type = "response" to clarify we want p(x) and not log(p(x)/(1-p(x))), for example
...
#Turn the balance category column into factors from char
...
Default.v3.train=Default.v3[train,]
...
Default.v3.test.output=Default.v3$default[test]
...
logreg.fit5.pred[logreg.fit5.prob>.5]="Yes"
logreg.fit5.pred
